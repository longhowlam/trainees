...
De traditionele manier in 'Base-R' om kolommen te wijzigen is om de `$` operator te gebruiken. 
...
Maar met `mutate` uit het dplyr package kan je dit veel eleganter doen
...
Soms wil je data ontdubbelen, dit kan je in R doen met de functie `distinct`. je kan distinct op alle kolommen toepassen of alleen op een bepaald aantal kolommen
...
Het omgekeerde van gather is spread.
...
In R kun je factor variabelen (categorische variabelen) bewerken met functies uit het `forcats` package. We geven een aantal voorbeelden op random gegenereerde data.
...
Analyzes op categorische variabelen met heel veel levels is soms niet handig. Misschien wil je bepaalde levels samenvegen.Dit smenvoegen van levels kan op aantallen (weinig voorkomende levels) of handmatig.
...
Naast de statistische grafieken die je in R kan maken met ggplot of traditionele plot functies, kan je in R ook interactieve grafieken maken. We geven in deze sectie een aantal voorbeelden.
